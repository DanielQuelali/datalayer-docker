...
docker \
  ...
  datalayer/spitfire-ambari:centos-6 "$@"
...
then
  ...
  export NAKED_IP=$(docker inspect --format="{{.NetworkSettings.IPAddress}}" datalayer-ambari)
  echo
  ...
  echo -e $YELLOW"  ssh -p 2222 root@localhost /start-spitfire-ambari.sh"$NOCOLOR
  echo -e $YELLOW"    (password is 'datalayer')"$NOCOLOR
  echo
fi
