...
then
  $ZEPPELIN_HOME/bin/datalayer-spitfire start "$@"
else
  $ZEPPELIN_HOME/bin/datalayer-spitfire-daemon start "$@"
  print_info
  tail -f $ZEPPELIN_HOME/logs/datalayer-spitfire.log
fi

echo
echo
echo -e $YELLOW"Bye bye Spitfire user... Hope to seen you back soon!"$NOCOLOR
echo
echo
